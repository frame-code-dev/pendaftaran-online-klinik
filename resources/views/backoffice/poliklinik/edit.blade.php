    ...
    <div class="p-4 sm:ml-64 pt-20 h-screen">
        <section class="p-5 overflow-y-auto mt-5">
            ...
            <div class="card bg-white p-5 mt-4 border rounded-md w-full relative">
                <form action="{{ route('poliklinik.update',$poliklinik->id) }}" method="POST" class="w-full mx-auto space-y-4" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                    <div>
                        <x-label-default for="" value="Nama Poliklinik"></x-label-default>
                        <x-input-default name="name" type="text" value="{{ old('name',$poliklinik->name) }}" placeholder="Masukkan Nama Poliklinik"></x-input-default>
                    </div>
                    <div>
                        <x-label-default for="" value="Keterangan">Keterangan</x-label-default>
                        <x-input-textarea id="keterangan" rows="5" name="keterangan" type="text" placeholder="Masukkan Keterangan">{{ old('keterangan',$poliklinik->keterangan) }}</x-input-textarea>
                    </div>
                    <div>
                        <x-label-default>Gambar</x-label-default>
                        <div>
                            <figure class="max-w-lg">
                                <img src="{{ $poliklinik->gambar != null ? asset('storage/poliklinik/'.$poliklinik->gambar) : 'https://flowbite.com/docs/images/examples/image-1@2x.jpg' }}" class="h-96 max-w-full rounded-lg photosPreview">
