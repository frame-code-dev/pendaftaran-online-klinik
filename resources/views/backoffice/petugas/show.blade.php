        ...
        <section class="p-5 overflow-y-auto mt-5">
            ...
            <div class="card bg-white p-5 mt-4 border rounded-md w-full relative">
                <div class="flex items-start content-start gap-4">
                    <div class="w-1/2">
                        <img class="rounded-md w-96 h-96 mx-auto" src="{{ $petugas->gambar != null ? asset('storage/petugas/'.$petugas->gambar) : 'https://flowbite.com/docs/images/examples/image-2@2x.jpg' }}" alt="image description">
                    </div>
                        ...
                        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                            <tbody class="border p-4 w-full">
                                ...
                                <tr class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    ...
                                    <td width="1%">:</td>
                                    <td class="font-bold">
                                        <span class="cursor-pointer bg-{{ $petugas->status == 'aktif' ? 'green' : 'red' }}-100 text-{{ $petugas->status == 'aktif' ? 'green' : 'red' }}-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-{{ $petugas->status == 'aktif' ? 'green' : 'red' }}-900 dark:text-{{ $petugas->status == 'aktif' ? 'green' : 'red' }}-300">{{ ucwords($petugas->status) }}</span>
